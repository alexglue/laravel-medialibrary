...
class ConversionCollection extends Collection
{
	...
	{
		$conversion = $this->first(
			function ($key, Conversion $conversion) use ($name)
			{
	...
	protected function addConversionsFromRelatedModel(Media $media)
	{
		...
		{
			/** @var $model HasMediaTrait|HasMediaConversions */
	...
	public function getConversions($collectionName = '')
	{
		...
		return $this->filter(
			function (Conversion $conversion) use ($collectionName)
			{
	...
	{
		return $this->getConversions($collectionName)->filter(
